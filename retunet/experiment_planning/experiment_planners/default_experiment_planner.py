...
from retunet.configuration import ANISO_THRESHOLD
from retunet.experiment_planning.experiment_planners.network_topology import get_pool_and_conv_props
from retunet.imageio.reader_writer_registry import determine_reader_writer_from_dataset_json
from retunet.paths import RetUNet_raw, RetUNet_preprocessed
from retunet.preprocessing.normalization.map_channel_name_to_normalization import get_normalization_scheme
from retunet.preprocessing.resampling.default_resampling import resample_data_or_seg_to_shape, compute_new_shape
from retunet.utilities.dataset_name_id_conversion import maybe_convert_to_dataset_name
...
from retunet.utilities.utils import get_filenames_of_train_images_and_targets
...
class ExperimentPlanner(object):
    def __init__(self, dataset_name_or_id: Union[str, int],
        ...
        self.raw_dataset_folder = join(RetUNet_raw, self.dataset_name)
        preprocessed_folder = join(RetUNet_preprocessed, self.dataset_name)
    ...
    def get_plans_for_configuration(self,
        ...
        batch_size = max(min(batch_size//4, bs_corresponding_to_5_percent), self.UNet_min_batch_size)
    ...
    def plan_experiment(self):
        ...
        shutil.copy(join(self.raw_dataset_folder, 'dataset.json'),
                    join(RetUNet_preprocessed, self.dataset_name, 'dataset.json'))
    ...
    def save_plans(self, plans):
        ...
        plans_file = join(RetUNet_preprocessed, self.dataset_name, self.plans_identifier + '.json')
        ...
        maybe_mkdir_p(join(RetUNet_preprocessed, self.dataset_name))
        save_json(plans, plans_file, sort_keys=False)
        print(f"Plans were saved to {join(RetUNet_preprocessed, self.dataset_name, self.plans_identifier + '.json')}")
