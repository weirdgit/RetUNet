...
from retunet.experiment_planning.dataset_fingerprint.fingerprint_extractor import DatasetFingerprintExtractor
from retunet.experiment_planning.experiment_planners.default_experiment_planner import ExperimentPlanner
from retunet.experiment_planning.verify_dataset_integrity import verify_dataset_integrity
from retunet.paths import RetUNet_raw, RetUNet_preprocessed
from retunet.utilities.dataset_name_id_conversion import convert_id_to_dataset_name
...
def extract_fingerprint_dataset(dataset_id: int,
    ...
    if check_dataset_integrity:
        verify_dataset_integrity(join(RetUNet_raw, dataset_name), num_processes)
...
def extract_fingerprints(dataset_ids: List[int], fingerprint_extractor_class_name: str = 'DatasetFingerprintExtractor',
    ...
    """
    fingerprint_extractor_class = recursive_find_python_class(join(retunet.__path__[0], "experiment_planning"),
                                                              fingerprint_extractor_class_name,
                                                              current_module="retunet.experiment_planning")
...
def plan_experiments(dataset_ids: List[int], experiment_planner_class_name: str = 'ExperimentPlanner',
    ...
    overwrite_target_spacing ONLY applies to 3d_fullres and 3d_cascade fullres!
    """
    experiment_planner = recursive_find_python_class(join(retunet.__path__[0], "experiment_planning"),
...
def preprocess_dataset(dataset_id: int,
    ...
    plans_file = join(RetUNet_preprocessed, dataset_name, plans_identifier + '.json')
    ...
    maybe_mkdir_p(join(RetUNet_preprocessed, dataset_name, 'gt_segmentations'))
    dataset_json = load_json(join(RetUNet_raw, dataset_name, 'dataset.json'))
    dataset = get_filenames_of_train_images_and_targets(join(RetUNet_raw, dataset_name), dataset_json)
    # only copy files that are newer than the ones already present
    for k in dataset:
        copy_file(dataset[k]['label'],
                  join(RetUNet_preprocessed, dataset_name, 'gt_segmentations', k + dataset_json['file_ending']),
                  update=True)
