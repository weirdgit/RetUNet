...
from batchgenerators.utilities.file_and_folder_operations import subfiles, join, save_json, load_json, \
    isfile
from retunet.configuration import default_num_processes
from retunet.imageio.base_reader_writer import BaseReaderWriter
from retunet.imageio.reader_writer_registry import determine_reader_writer_from_dataset_json, \
    determine_reader_writer_from_file_ending
from retunet.imageio.simpleitk_reader_writer import SimpleITKIO
# the Evaluator class of the previous nnU-Net was great and all but man was it overengineered. Keep it simple
from retunet.utilities.json_export import recursive_fix_for_json_export
from retunet.utilities.plans_handling.plans_handler import PlansManager
...
def calculate_95hd(surface1, surface2):
    """
    计算95th percentile Hausdorff Distance
    :param surface1: 第一个表面的点集，形状为(N, D)，N为点数，D为维度
    :param surface2: 第二个表面的点集，形状为(M, D)，M为点数，D为维度
    :return: 95HD
    """
    if surface1.size == 0 or surface2.size == 0:
        # 可以返回一个大数，表示距离很大，或者返回np.inf
        return np.sum(np.abs(surface2))+np.sum(np.abs(surface1))
    tree1 = cKDTree(surface1)
    tree2 = cKDTree(surface2)
    
    # 从surface1到surface2的最短距离
    dists_1, _ = tree1.query(surface2, k=1)
    # 从surface2到surface1的最短距离
    dists_2, _ = tree2.query(surface1, k=1)
    
    # 合并两个方向的距离并计算95th percentile
    all_dists = np.concatenate([dists_1, dists_2])
    hd95 = np.percentile(all_dists, 95)
    
    return hd95


def calculate_asd(surface1, surface2):
    """
    计算Average Surface Distance
    :param surface1: 第一个表面的点集，形状为(N, D)
    :param surface2: 第二个表面的点集，形状为(M, D)
    :return: ASD
    """
    if surface1.size == 0 or surface2.size == 0:
        # 可以返回一个大数，表示距离很大，或者返回np.inf
    ...
    # 从surface1到surface2的平均最短距离
    dists_1, _ = tree1.query(surface2, k=1)
    mean_dist_1 = np.mean(dists_1)
    
    # 从surface2到surface1的平均最短距离
    dists_2, _ = tree2.query(surface1, k=1)
    mean_dist_2 = np.mean(dists_2)

    # 计算平均距离
    asd = (mean_dist_1 + mean_dist_2) / 2

    return asd


def extract_surface_points(segmentation, label):
    """
    从分割图中提取给定标签的边界点，适用于2D和3D。
    :param segmentation: 分割图，形状为(H, W)或(D, H, W)
    :param label: 要提取边界的类别标签
    :return: 该类别的边界点的坐标列表
    """
    boundaries = find_boundaries(segmentation == label, mode='thick').astype(np.uint8)
...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    for r in labels_or_regions:
        ...
        if 1 not in mask_ref:
            print(f"{1} in mask_ref:{1 in mask_ref}")
        if 1 not in mask_pred:
            print(f"{1} in mask_pred:{1 in mask_pred}")

        surface_image = extract_surface_points(mask_ref, 1)
        surface_prediction = extract_surface_points(mask_pred, 1)

        hd95 = calculate_95hd(surface_image, surface_prediction)
        asd = calculate_asd(surface_image, surface_prediction)
        results['metrics'][r]['95HD'] = hd95
...
def compute_metrics_wrapper(args):
    # 解包参数
    ref_file, pred_file, reader_writer, labels_or_regions, ignore = args
    
    # 调用原始的 compute_metrics 函数
    result = compute_metrics(ref_file, pred_file, reader_writer, labels_or_regions, ignore)
    
    # 更新进度
    compute_metrics_wrapper.counter += 1
    print(f"Progress: {compute_metrics_wrapper.counter} files processed.")
    
    return result
compute_metrics_wrapper.counter = 0
