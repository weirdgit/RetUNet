...
from retunet.ensembling.ensemble import ensemble_crossvalidations
from retunet.evaluation.accumulate_cv_results import accumulate_cv_results
from retunet.evaluation.evaluate_predictions import compute_metrics_on_folder, load_summary_json
from retunet.paths import RetUNet_preprocessed, RetUNet_raw, RetUNet_results
from retunet.postprocessing.remove_connected_components import determine_postprocessing
from retunet.utilities.file_path_utilities import maybe_convert_to_dataset_name, get_output_folder, \
...
def filter_available_models(model_dict: Union[List[dict], Tuple[dict, ...]], dataset_name_or_id: Union[str, int]):
    ...
    for trained_model in model_dict:
        plans_manager = PlansManager(join(RetUNet_preprocessed, maybe_convert_to_dataset_name(dataset_name_or_id),
        ...
        if trained_model['configuration'] not in plans_manager.available_configurations:
            print(f"Configuration {trained_model['configuration']} not found in plans {trained_model['plans']}.\n"
                  f"Inferred plans file: {join(RetUNet_preprocessed, maybe_convert_to_dataset_name(dataset_name_or_id), trained_model['plans'] + '.json')}.")
            continue
...
def find_best_configuration(dataset_name_or_id,
    ...
    if allow_ensembling:
        for i in range(len(allowed_trained_models)):
            for j in range(i + 1, len(allowed_trained_models)):
                ...
                output_folder_ensemble = join(RetUNet_results, dataset_name, 'ensembles', identifier)
                ...
                compute_metrics_on_folder(join(RetUNet_preprocessed, dataset_name, 'gt_segmentations'),
    ...
    determine_postprocessing(all_results[best_key]['source'], join(RetUNet_preprocessed, dataset_name, 'gt_segmentations'),
    ...
    save_json(return_dict, join(RetUNet_results, dataset_name, 'inference_information.json'))  # save this so that we don't have to run this
    ...
    # print it
    print_inference_instructions(return_dict, instructions_file=join(RetUNet_results, dataset_name, 'inference_instructions.txt'))
    return return_dict
