...
def find_candidate_datasets(dataset_id: int):
    ...
    if RetUNet_preprocessed is not None and isdir(RetUNet_preprocessed):
        candidates_preprocessed = subdirs(RetUNet_preprocessed, prefix=startswith, join=False)
    else:
        candidates_preprocessed = []

    if RetUNet_raw is not None and isdir(RetUNet_raw):
        candidates_raw = subdirs(RetUNet_raw, prefix=startswith, join=False)
    else:
        candidates_raw = []

    candidates_trained_models = []
    if RetUNet_results is not None and isdir(RetUNet_results):
        candidates_trained_models += subdirs(RetUNet_results, prefix=startswith, join=False)
...
def convert_id_to_dataset_name(dataset_id: int):
    ...
    if len(unique_candidates) > 1:
        raise RuntimeError("More than one dataset name found for dataset id %d. Please correct that. (I looked in the "
                           "following folders:\n%s\n%s\n%s" % (dataset_id, RetUNet_raw, RetUNet_preprocessed, RetUNet_results))
    if len(unique_candidates) == 0:
        raise RuntimeError(f"Could not find a dataset with the ID {dataset_id}. Make sure the requested dataset ID "
                           ...
                           f"RetUNet_preprocessed={os.environ.get('RetUNet_preprocessed') if os.environ.get('RetUNet_preprocessed') is not None else 'None'}\n"
                           f"RetUNet_results={os.environ.get('RetUNet_results') if os.environ.get('RetUNet_results') is not None else 'None'}\n"
                           f"RetUNet_raw={os.environ.get('RetUNet_raw') if os.environ.get('RetUNet_raw') is not None else 'None'}\n"
