...
from retunet.preprocessing.resampling.utils import recursive_find_resampling_fn_by_name
import retunet
from batchgenerators.utilities.file_and_folder_operations import load_json, join

from retunet.imageio.reader_writer_registry import recursive_find_reader_writer_by_name
from retunet.utilities.find_class_by_name import recursive_find_python_class
from retunet.utilities.label_handling.label_handling import get_labelmanager_class_from_plans
...
if TYPE_CHECKING:
    from retunet.utilities.label_handling.label_handling import LabelManager
    from retunet.imageio.base_reader_writer import BaseReaderWriter
    from retunet.preprocessing.preprocessors.default_preprocessor import DefaultPreprocessor
...
class ConfigurationManager(object):
    ...
    def preprocessor_class(self) -> Type[DefaultPreprocessor]:
        preprocessor_class = recursive_find_python_class(join(retunet.__path__[0], "preprocessing"),
                                                         self.preprocessor_name,
                                                         current_module="retunet.preprocessing")
...
if __name__ == '__main__':
    from retunet.paths import RetUNet_preprocessed
    from retunet.utilities.dataset_name_id_conversion import maybe_convert_to_dataset_name

    plans = load_json(join(RetUNet_preprocessed, maybe_convert_to_dataset_name(3), 'nnUNetPlans.json'))
