...
from retunet.evaluation.evaluate_predictions import load_summary_json
from retunet.paths import RetUNet_results
from retunet.utilities.dataset_name_id_conversion import maybe_convert_to_dataset_name, convert_dataset_name_to_id
...
def summarize(input_file, output_file, folds: Tuple[int, ...], configs: Tuple[str, ...], datasets, trainers):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(',mean\n')
        valid_entries = txt[:, 4] == RetUNet_results
...
if __name__ == '__main__':
    ...
    all_results_file= join(RetUNet_results, 'customDecResults.csv')
    ...
    folds = (0, 1, 2, 3, 4)
    configs = ("3d_fullres", )
    output_file = join(RetUNet_results, 'customDecResults_summary5fold.csv')
    summarize(all_results_file, output_file, folds, configs, datasets, use_these_trainers)

    folds = (0, )
    configs = ("3d_fullres", )
    output_file = join(RetUNet_results, 'customDecResults_summaryfold0.csv')
    summarize(all_results_file, output_file, folds, configs, datasets, use_these_trainers)
